...
from embedding import ConditionalEmbedding, MNISTEmbedding
...
def train(params:argparse.Namespace):
    ...
    for epc in range(lastepc, params.epoch):
        ...
        if (epc + 1) % params.interval == 0:
            ...
            with torch.no_grad():
                lab = val_batch[params.condkey].to(device)
                cemb = cemblayer(lab)
...
def main():
    ...
    parser.add_argument('--datakey', type=str, required=True, help='which of the data keys is the one we want to generate')
    parser.add_argument('--condkey', type=str, required=True, help='which of the data keys is the one we use for conditioning')
