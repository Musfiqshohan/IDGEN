...
from Baselines.NeuralCausalAbstractionsMain.src.ds.causal_graph import CausalGraph
from Baselines.NeuralCausalAbstractionsMain.src.datagen import SCMDataset, get_transform
from Baselines.NeuralCausalAbstractionsMain.src.datagen import SCMDataTypes as sdt
from Baselines.NeuralCausalAbstractionsMain.src.metric.model_utility import load_model
from Baselines.NeuralCausalAbstractionsMain.src.run import BaseRunner
from Baselines.NeuralCausalAbstractionsMain.src.scm.scm import expand_do
from Baselines.NeuralCausalAbstractionsMain.src.pipeline.repr_pipeline import RepresentationalPipeline
from Baselines.NeuralCausalAbstractionsMain.src.scm.repr_nn.representation_nn import RepresentationalNN
import Baselines.NeuralCausalAbstractionsMain.src.metric.visualization as vis
from path_constant import project_root
# from .base_runner import BaseRunner
import torch

class NapkinRunner(BaseRunner):
    """
    ...
    def run_repmodels(self, hyperparams, d, dat_set, rep_m, img_query, gpu):
        # Check if representation model already exists
        #'gen=NapkinMNISTDataGenerator-pipeline=GANPipeline-model=GAN_NCM-n_samples=10000-trial_index=0'
        if os.path.isfile(f'{d}/best_rep.th'):
            ...
            # Train representational model for W1
            rep_trainer, rep_checkpoint = self.create_trainer(d, f"rep_model", hyperparams['max-epochs'],
            ...
            if img_query:
                img_sample = dat_set.get_image_batch(64)
                DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                rep_m.model= rep_m.model.to(DEVICE)
                regen_sample = rep_m.model(img_sample)
                for img_var in img_sample:
                    if hyperparams["verbose"]:
                        vis.show_image_grid(img_sample[img_var])
    ...
    def run(self, exp_name, n, trial_index, hyperparams=None, gpu=None,
        ...
        d = f'{project_root}/Baselines/NeuralCausalAbstractionsMain/src/out/%s/%s' % (exp_name, key)  # name of the output directory
    ...
    # with self.lock(f'{d}/lock', lockinfo) as acquired_lock:
        ...
        try:
            ...
            if os.path.isfile(f'{d}/best.th'):
                print('here?', f'{d}/best.th')
                print('[Continuing Training]', d)
                # return
            ...
            if img_query:
                ...
                cg = CausalGraph.read(f"{project_root}/Baselines/NeuralCausalAbstractionsMain/dat/cg/{dat_m.cg}.cg")  # Causal diagram object
                v_size = dat_m.v_size
                v_type = dat_m.v_type
                ...
                cg = CausalGraph.read(f"{project_root}/Baselines/NeuralCausalAbstractionsMain/dat/cg/{cg_name}.cg")  # Causal diagram object
                dat_set = SCMDataset(  # Convert data to a Torch Dataset object
                    dat_m, n, augment_transform=get_transform(hyperparams["transform"], hyperparams["img-size"]))
            ...
            if hyperparams['repr'] != "none":
                ...
                print('Starting rep')
                rep_m= self.run_repmodels(hyperparams, d, dat_set, rep_m, img_query, gpu)
                print('Done with all rep')
            ...
            print("RNCM is not loaded")


            print('RNCM training started')
            ...
            # Updated: No files moving will be done
            # Move out/*/* to err/*/*/#
